...
// Averaged fuel level reading to counter the sloshing effect
...
#define Version "Fuel Temp Bar V14"
...
const int Fuel_Min = 0;       // set the size of the fuel tank
const int Fuel_Max = 45;
const int Temp_Min = 20;
const int Temp_Max = 120;
...
// Input variables
int Temp_Celsius, Fuel_Litres, Dummy;
int Battery_Volts, Alternator_Volts;
uint32_t Fuel_Raw;
float Fuel_Average, Temp_Float, V1, V2;
...
uint32_t Long_Loop_Time, Short_Loop_Time, Loop_Count;
...
void Read_Fuel() {
  ...
  // in order to average the value over a large number of loops
...
void Draw_Fuel_Meter() {
  ...
  if (Calibration_Mode == false) {
    ...
    if (Fuel_Litres >= Warning_Fuel) {
	  ...
	  if (Fuel_Litres < (Warning_Fuel * 1.5)) {
...
void Draw_Temp_Meter() {
  ...
  Temp_Float = float(analogRead(Temp_Pin)) + 0.5;
  ...
  if (Calibration_Mode == false) {
    ...
    // Constrain the temp sensor reading
...
void Display_Warning_Text() {
  ...
  */
  ...
  switch (Status_Priority) {  // start of the switch statement
